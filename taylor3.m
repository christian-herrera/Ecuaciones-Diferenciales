...
##                                                                            ##
## Funcionamiento:                                                            ##
##   El algoritmo recibe 4 o 5 valores y tres funcion (Una es nuestra funcion ##
##   principal, f(x,y) y la segunda es la derivada de esta funcion, y la      ##
##   tercera es la derivada segunda de nuestra funcion principal).  Este      ##
...
##   El algoritmo se basa en el Metodo de Taylor de Orden 3.                  ##
##                                                                            ##
##   Funciona realizando calculos de forma recursiva. Su ecuacion para hacer  ##
##   los calculos es la siguiente:                                            ##
## Y(i+1)=Y(i) + f(X(i),Y(i)*h + f'(X(i),Y(i))*(h^2)/2+f''(X(i),Y(i))*(h^3)/6 ##
##                                                                            ##
...
function [result, matriz] = taylor3(x0, y0, f, der1_f, der2_f, h, aprox, mostrar_tabla=1)
  clc;
  ...
  if(mod(n,1) != 0)
    ...
    for(i=2:n+1)
      X(i) = X(i-1) + h;
      Y(i) = Y(i-1) + f(X(i-1), Y(i-1))*h + der1_f(X(i-1), Y(i-1))*((h^2)/2) + der2_f(X(i-1), Y(i-1))*((h^3)/6);
    endfor;
